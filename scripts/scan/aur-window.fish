...
for pkg in (string split \n -- "$raw")
    ...
    set -l evt_count (aur_pkg_event_count_in_events $events $pkg)
    ...
    if test $evt_count -ge 2
        aur_finding_add window_repeat_updates "$pkg|$evt_count"
        aur_log "           repeat: $evt_count updates during window — earliest may predate takedown"
        set -l repeat_lines (aur_pkg_event_lines_from_events $events $pkg)
        for rline in $repeat_lines
            aur_log "             $rline"
        end
    end
    aur_log "           $line"
