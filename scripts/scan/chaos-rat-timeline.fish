...
aur_begin_report_if_requested chaos-rat-timeline-

aur_log "=== Chaos RAT pacman install timeline ==="
if test $AUR_OPT_all_time = true
    aur_log "Scanning pacman logs for Chaos RAT packages (all dates; --all-time)"
else
    aur_log "Scanning pacman logs for Chaos RAT packages, $AUR_CHAOS_RAT_WINDOW_LABEL"
end
aur_log ""
...
set -l events (mktemp)
if test $AUR_OPT_all_time = true
    # All-time: collect every ALPM install line, then intersect with the chaos list.
    ...
    aur_collect_chaos_rat_window_alpm_events_all $events
end
...
if test $hit_count -eq 0
    aur_log "[OK] No Chaos RAT packages in pacman logs during $AUR_CHAOS_RAT_WINDOW_LABEL"
else
    aur_summary_set chaos_rat_timeline_hits $hit_count
    aur_log "[FOUND] $hit_count Chaos RAT timeline hit(s):"
    for hit in (string split \n -- "$raw")
        test -n "$hit"; or continue
        aur_finding_add chaos_rat_timeline_hits $hit
        aur_log "  $hit"
    end
    set -l repeat_events (mktemp)
    set -l saved_all_time $AUR_OPT_all_time
    set -g AUR_OPT_all_time false
    aur_collect_chaos_rat_window_alpm_events_all $repeat_events
    set -g AUR_OPT_all_time $saved_all_time
    aur_report_timeline_repeat_updates $repeat_events $list_file chaos_rat_timeline_repeat_updates chaos_rat_timeline_repeat_updates "during $AUR_CHAOS_RAT_WINDOW_LABEL"
    rm -f $repeat_events
    aur_log ""
