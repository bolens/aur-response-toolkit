...
# comm -12: intersection of installed foreign pkgs and known infected list.
...
if test (count $found) -gt 0
    ...
    for pkg in $found
        aur_finding_add installed_infected $pkg
        ...
        if aur_install_in_compromise_window $pkg
            ...
            aur_finding_add installed_high_risk $pkg
            aur_log "  [HIGH]   $pkg"
            aur_log "           installed: $install_date | reason: $install_reason"
        else
            # Infected list match but install date outside window — may be pre-existing or reinstalled later.
