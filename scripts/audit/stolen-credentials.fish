...
if test $AUR_OPT_report = true; and not set -q AUR_REPORT_FILE[1]
    aur_begin_report stolen-credentials-
end
...
function audit_find --argument-names base pattern desc
    ...
    for f in (aur_find $base -name $pattern 2>/dev/null)
...
aur_find $AUR_DEV_ROOT -maxdepth 5 \( -name '.env' -o -name 'secrets.env' -o -name 'stack.env' -o -name 'shared.env' -o -name 'harbor.yml' \) 2>/dev/null >$env_list
set -l env_count 0
while read -l f
...
if test (count $AUR_AUDIT_HISTORY_FILES) -gt 0
    ...
    aur_log "  Scrub after rotation: fish (aur_script_path recovery/scrub-history.fish) --all-shells"
else
