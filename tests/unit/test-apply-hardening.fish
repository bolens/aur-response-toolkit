...
set -l out (env HOME=$tmp_home fish $AUR_RESPONSE_DIR/scripts/apply-hardening.fish 2>&1 | string collect)
assert_match "dry-run mentions apply" 'DRY-RUN' "$out"
assert_eq "npmrc unchanged" "# empty" (cat $tmp_home/.npmrc)

set -l out_apply (env HOME=$tmp_home fish $AUR_RESPONSE_DIR/scripts/apply-hardening.fish --apply 2>&1 | string collect)
