...
set -l raw (aur_parse_pkg_names (cat (test_fixture_path fetch/arch-list.html)) | string collect)
...
test_section "installed vs atomic arch list intersection"
...
set -l found (aur_installed_atomic_arch_pkgs (cat $infected))
assert_count "aur_installed_atomic_arch_pkgs match" 1 $found
assert_eq "only beef matches" beef $found[1]
set -e AUR_TEST_INSTALLED_LIST
rm -f $installed $infected
