...
begin
    aur_file_has_hook_pattern (test_fixture_path pkgbuilds/pkgbuild.malicious)
    assert_status "malicious PKGBUILD detected" 0
end
begin
    aur_file_has_hook_pattern (test_fixture_path pkgbuilds/pkgbuild.clean)
    assert_status "clean PKGBUILD passes" 1
end

test_section "env and history secrets"
begin
    aur_env_has_secrets (test_fixture_path env/env-secrets.env)
    assert_status "env with TOKEN detected" 0
end
begin
    aur_env_has_secrets (test_fixture_path env/env-clean.env)
    assert_status "clean env passes" 1
end

assert_eq "history secret hits" 2 (aur_history_secret_hits (test_fixture_path history/history-secrets.txt))
...
aur_state_init
aur_summary_set atomic_arch_timeline_hits 3
aur_summary_set list_added 2
assert_eq "state get timeline" 3 (aur_state_get atomic_arch_timeline_hits)
...
assert_eq "state load summary" 3 $AUR_SUMMARY_atomic_arch_timeline_hits
